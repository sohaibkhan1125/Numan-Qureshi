...
const API_URL = `https://v2.convertapi.com/convert/webp/to/svg?Secret=${API_KEY}`;
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        if (data.Files?.length) {
          for (const f of data.Files) {
            const blob = await b64ToBlob(f.FileData, 'image/svg+xml');
  ...
  return (
    <div className="min-h-screen bg-[#fbfcfe] pt-32 pb-16 px-4">
      <style
        ...
        .webp-svg-drop-active { animation: pulse-border 1.2s ease-in-out infinite; border-style: solid !important; }
        .webp-svg-shimmer {
          background: linear-gradient(90deg, #4F6EF7 25%, #7C3AED 50%, #4F6EF7 75%);
          background-size: 400px 100%;
          animation: webp-svg-shimmer-move 1.4s infinite;
        }
        @keyframes webp-svg-shimmer-move {
          0% { background-position: -400px 0; }
          100% { background-position: 400px 0; }
        }
        .custom-scrollbar-webpsvg::-webkit-scrollbar { width: 5px; }
        .custom-scrollbar-webpsvg::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 10px; }
        .custom-scrollbar-webpsvg::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; }
        .custom-scrollbar-webpsvg::-webkit-scrollbar-thumb:hover { background: #4F6EF7; }
      ...
      <div className="max-w-4xl mx-auto text-center">
        <header className="mb-12">
          <h1 className="text-4xl sm:text-5xl font-black text-[#1E2A5E] mb-4 tracking-tight">WEBP to SVG</h1>
          <p className="text-gray-500 text-lg font-medium opacity-80 max-w-2xl mx-auto text-[#1E2A5E]/60">
            Professional vector synthesis. Convert WEBP rasters into optimized, scalable SVG graphics in seconds.
          </p>
        </header>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)] border border-gray-100 transition-all duration-500 overflow-hidden text-left">
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
            <div className="p-10 sm:p-14 bg-gray-50/30">
              <label className="block text-[10px] font-black uppercase tracking-[4px] text-gray-300 mb-8 leading-none">
                Step 1: Raster Loading
              </label>
              <div
                className={`border-2 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center p-14 cursor-pointer transition-all bg-white hover:border-[#4F6EF7] hover:bg-blue-50/20 group relative ${
                  isDropActive ? 'webp-svg-drop-active border-[#4F6EF7]' : 'border-gray-100'
                }`}
              ...
              >
                ...
                <div className="text-center">
                  <p className="text-[#1E2A5E] font-black text-lg italic mb-1">Select WEBP Files</p>
                  <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mt-1">
                    Raster Ready · Multi-File
                  </p>
                </div>
                <input
                  ref={fileInputRef}
              ...
              <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <p className="text-gray-400 text-[11px] font-bold uppercase tracking-wide leading-relaxed">
                    High-Res Vector Encoding Logic
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-10 sm:p-14 bg-white">
              <label className="block text-[10px] font-black uppercase tracking-[4px] text-gray-300 mb-8 leading-none">
                Step 2: Synthesis Management
              </label>
              {selectedFiles.length === 0 ? (
                <div className="text-center py-20 opacity-20">
                  <p className="text-[#1E2A5E] font-black uppercase text-[10px] tracking-[5px]">Waiting for Rasters</p>
                </div>
              ) : (
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar-webpsvg text-left text-[#1E2A5E]">
                  {selectedFiles.map((item) => (
                    <div
                      key={item.id}
                      className="img-card bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group"
                    >
          ...
          {isConverting && (
            <div className="p-12 bg-gray-50 border-t border-gray-100 text-center">
              <div className="flex flex-col items-center gap-6">
                ...
                <div>
                  <p className="text-[#1E2A5E] font-extrabold text-sm uppercase tracking-[5px]">PARSING GRAPH NODES...</p>
                  <p className="text-gray-400 text-[10px] mt-2 uppercase font-bold tracking-widest leading-none">
                    Mapping Raster Grids to Scalable SVG Paths
                  </p>
                </div>
              </div>
              <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden mt-10 max-w-sm mx-auto">
                <div className="h-full webp-svg-shimmer rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
              </div>
            </div>
          )}
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-gray-400 text-[10px] font-bold mt-2 uppercase tracking-widest opacity-60">
                    High-Resolution Vector Assets Generated
                  </p>
                </div>
                <button
                  type="button"
                ...
                {generatedImages.map((img) => (
                  <div key={img.url} className="img-card bg-gray-50/50 border border-gray-100 rounded-2xl overflow-hidden group">
                    <div className="aspect-[3/2] overflow-hidden relative">
                      <img src={img.url} alt="" className="w-full h-full object-contain p-4" />
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {[
            {
              t: 'Elite Vector Engine',
              d: 'Global cloud nodes ensure your WEBP rasters are synthesized into high-density SVG assets in seconds without detail loss.',
            },
            {
              t: 'Sequential Logic',
              d: 'Easily upload multiple WEBP files at once. Our system handles massive image volumes with sequential cloud threads for maximum stability.',
            },
            {
              t: 'Secure Asset Stream',
              d: 'TLS 1.3 encryption ensures your image files are isolated and purged immediately post-synthesis.',
            },
          ].map((x) => (
