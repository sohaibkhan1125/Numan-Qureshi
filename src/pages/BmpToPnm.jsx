...
const BmpToPnm = () => {
    ...
    const API_URL = 'https://v2.convertapi.com/convert/bmp/to/pnm?Secret=' + API_KEY;

    // ── STATE ───────────────────────────────────────────
    const [selectedFiles, setSelectedFiles] = useState([]); // array of { file, id }
    const [generatedFiles, setGeneratedFiles] = useState([]); // { name, url, blob }
    ...
    const b64ToBlob = (b64, mime = 'image/x-portable-anymap') => {
    ...
    const downloadAll = () => {
        generatedFiles.forEach((f, i) => {
            setTimeout(() => downloadFile(f.url, f.name), i * 400);
        });
    };
    ...
    return (
        <div className="min-h-screen bg-[#fbfcfe]">
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                .pnm-badge { background: linear-gradient(135deg, #F59E0B, #D97706); }
                .asset-card { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
                .asset-card:hover { transform: translateY(-5px); box-shadow: 0 15px 30px -10px rgba(0, 0, 0, 0.1); }
            ...
            <div className="max-w-4xl mx-auto px-4 py-16 text-center">
                <header className="mb-12">
                    <h1 className="text-4xl sm:text-5xl font-black mb-4 tracking-tight text-[#1E2A5E]">BMP to PNM</h1>
                    <p className="text-[#1E2A5E]/60 text-lg font-bold max-w-2xl mx-auto uppercase tracking-wider">
                        High-fidelity format synthesis. Convert massive bitmap files into portable PNM assets in seconds.
                    </p>
                </header>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)] border border-gray-100 transition-all duration-500 overflow-hidden text-left">
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div className="p-10 sm:p-14 bg-gray-50/30">
                            ...
                            >
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mt-1">Cloud Ready · Multiple Assets</p>
                                </div>
                            ...
                            <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                                <div className="flex items-start gap-4">
                                    ...
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Raw Data Integrity Synthesis</p>
                                </div>
                                <div className="flex items-start gap-4">
                                    <div className="w-6 h-6 rounded-full bg-green-50 text-green-500 flex items-center justify-center flex-shrink-0 shadow-sm"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" /></svg></div>
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Portable Binary Stream Encoding</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white">
                            ...
                            {selectedFiles.length === 0 ? (
                                ...
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedFiles.map((item) => (
                                        <div key={item.id} className="asset-card bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group animate-in slide-in-from-right-2 duration-300">
                            ...
                            <div className="mt-10 pt-8 border-t border-gray-50 uppercase tracking-widest text-[#1E2A5E]">
                                ...
                                >
                                    {isConverting ? 'ENCODING...' : 'CONVERT TO PNM'}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {isConverting && (
                        <div className="p-12 bg-gray-50 border-t border-gray-100 text-center animate-in fade-in duration-500">
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[#1E2A5E] font-black text-sm uppercase tracking-[5px]">ENCODING RAW BLOCKS...</p>
                                    <p className="text-gray-400 text-[10px] mt-2 uppercase font-bold tracking-widest leading-none">Mapping Bitmap Buffers to Portable PNM Streams</p>
                                </div>
                            </div>
                    ...
                    {/* Results Section */}
                    {generatedFiles.length > 0 && !isConverting && (
                        <div className="p-10 sm:p-14 bg-white border-t border-gray-100 text-left">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <p className="text-gray-400 text-[10px] font-bold mt-2 uppercase tracking-widest opacity-60">High-Resolution PNM Assets Generated</p>
                                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
                                {generatedFiles.map((f, i) => (
                                    <div key={i} className="asset-card bg-gray-50 border border-gray-100 rounded-2xl p-6 relative group animate-in zoom-in-95 duration-500 delay-[50ms] shadow-sm hover:shadow-xl">
                                        <div className="flex flex-col items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl pnm-badge flex items-center justify-center text-white shadow-lg">
                                            ...
                                            <div className="text-center min-w-0 w-full">
                                                <p className="text-[#1E2A5E] font-black text-[10px] uppercase tracking-widest truncate">{f.name}</p>
                                                <p className="text-gray-400 text-[9px] font-bold uppercase mt-1 tracking-tighter capitalize leading-none">PNM Vector/Stream</p>
                                            </div>
                                            <button onClick={() => downloadFile(f.url, f.name)} className="mt-2 w-full py-2 bg-gray-100 group-hover:bg-[#4F6EF7] group-hover:text-white text-[#1E2A5E] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2">
                                                 <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                                 Download
                                            </button>
                                        </div>
                                    </div>
