...
const TiktokDownloader = () => {
  ...
  const startDownload = async () => {
    ...
    setDownloadLinks([]);
    setVideoInfo(null);
    setProgressMsg('Fetching TikTok information...');

    try {
      const encodedUrl = encodeURIComponent(url);
      const apiPath = `/api/v1/tiktok-media/info?url=${encodedUrl}`;
    ...
    <div className="bg-slate-50 min-h-screen pt-24 pb-12">
      ...
      <div className="container mx-auto px-4 md:px-6 mb-8 max-w-4xl">
        <nav className="flex items-center gap-2 text-sm text-slate-400 font-medium">
          ...
          <span>›</span>
          <span className="text-slate-600">TikTok Downloader</span>
        </nav>
      </div>

      <div className="container mx-auto px-4 md:px-6 max-w-4xl">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-800/10 text-slate-800 mb-6 group-hover:scale-110 transition-transform shadow-sm">
             <svg className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5v3a8 8 0 0 1-5-3v5.5a4 4 0 0 1-4 4z"/></svg>
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-slate-800 mb-4 tracking-tight">TikTok Downloader</h1>
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-10 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
          ...
          {!videoInfo && !isLoading && (
            <div className="flex flex-col gap-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">TikTok Video URL</label>
              ...
              <button
                ...
                className="w-full bg-slate-800 hover:bg-slate-900 text-white font-black text-lg py-4 rounded-xl transition-all shadow-lg shadow-slate-900/25 mt-4 active:scale-[0.98]"
              >
                Fetch Video
              </button>
            </div>
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <svg className="animate-spin -ml-1 mr-3 h-12 w-12 text-slate-800 mb-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <div className="flex flex-col gap-3 w-full">
                      {downloadLinks.map((linkObj, index) => (
                        <a
                           ...
                           target="_blank"
                           rel="noreferrer"
                           className="flex items-center justify-between bg-white border border-slate-200 p-4 rounded-xl hover:border-slate-800 hover:shadow-md transition-all active:scale-[0.99] no-underline decoration-none group"
                        >
                           <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${linkObj.type === 'audio' ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-800'}`}>
                                {linkObj.type === 'audio' ? (
                              ...
                              <div className="text-left">
                                ...
                                <span className="block text-slate-500 text-xs font-medium">TikTok Media</span>
                              </div>
                           </div>
                           <div className="bg-slate-50 text-slate-800 font-bold p-2 px-4 rounded-lg group-hover:bg-slate-800 group-hover:text-white transition-colors text-sm">
                             Download
                           </div>
                        </a>
                      ))}
