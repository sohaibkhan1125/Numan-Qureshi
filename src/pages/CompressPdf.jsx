...
const LEVEL_MAP = {
  low: { quality: 50, resolution: 144, label: 'Low', desc: 'High Quality', iconColor: 'text-[#22c55e]', iconBg: 'bg-[#dcfce7]', icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 11.388c.11.192.316.312.538.312h4.594V17a1 1 0 102 0v-5.3h4.594a.625.625 0 00.538-.312.625.625 0 00-.03-.66l-5.625-7.5a.625.625 0 00-1.002 0l-5.625 7.5a.625.625 0 00-.032.66z" clipRule="evenodd"/></svg> },
  medium: { quality: 35, resolution: 100, label: 'Medium', desc: 'Balanced', iconColor: 'text-[#3b82f6]', iconBg: 'bg-[#dbeafe]', icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z"/></svg> },
  high: { quality: 15, resolution: 72, label: 'High', desc: 'Maximum Compression', iconColor: 'text-[#ef4444]', iconBg: 'bg-[#fee2e2]', icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd"/></svg> },
};
...
const CompressPdf = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      // Use the modular API service
      const fileResult = await compressPdf(selectedFile, {
        quality: level.quality,
        resolution: level.resolution
      });

      // Handle the API result (ConvertAPI returns base64 in FileData)
      ...
      setTimeout(() => {
        setResult({
          ...
          filename: fileResult.FileName || selectedFile.name.replace('.pdf', '_compressed.pdf')
        });
        setStatus('success');
      ...
      clearInterval(progressTimer);
      setErrorMsg(err.message || 'An error occurred during compression. Please check your API key or network connection.');
      setStatus('error');
    }
  };
  ...
  return (
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans">
      <style>{`
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">Compress PDF</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Reduce your PDF file size instantly with ease</p>
        </header>
        ...
        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)]">
          ...
          {!selectedFile && (
            ...
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl pdf-badge-gradient flex items-center justify-center text-white shadow-xl group-hover:scale-105 transition-transform duration-300">
                <svg className="w-10 h-10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
              </div>
              <div className="text-center">
                <p className="text-[#1E2A5E] font-bold text-xl mb-2">Drag & drop PDF file here</p>
          ...
          {selectedFile && (
            <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between transition-all duration-500">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-2xl pdf-badge-gradient flex items-center justify-center text-white text-[10px] font-bold shadow-sm">PDF</div>
          ...
          {/* Configurations */}
          <div className="mt-14 h-px bg-slate-100 relative">
            <span className="absolute left-1/2 -top-3 -translate-x-1/2 bg-white px-4 text-slate-300 text-[10px] font-extrabold uppercase tracking-widest">Compression Level</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12">
            {Object.entries(LEVEL_MAP).map(([key, config]) => {
              ...
              return (
                <button 
                  ...
                  className={`border rounded-3xl p-6 text-left group transition-all duration-300 ${isSelected ? 'border-[#4F6EF7] bg-[#F8FBFF] shadow-[0_0_0_4px_rgba(79,110,247,0.1)]' : 'border-slate-200 opacity-80 hover:opacity-100'}`}
                >
                  <div className={`w-10 h-10 rounded-2xl ${config.iconBg} flex items-center justify-center ${config.iconColor} mb-4 group-hover:scale-110 transition-transform`}>
                    {config.icon}
                  </div>
                  <p className="text-[#1E2A5E] font-extrabold text-base">{config.label}</p>
                  <p className="text-slate-400 text-xs font-medium mt-1">{config.desc}</p>
                </button>
              )
            })}
          </div>
          ...
          {status === 'processing' && (
            <div ref={progressRef} className="mt-10 p-8 bg-slate-50 border border-slate-100 rounded-[2rem] text-center transition-all duration-500">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <p className="text-[#1E2A5E] font-extrabold text-sm tracking-widest uppercase">SHRINKING YOUR PDF...</p>
              </div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div className="text-left">
                    <p className="text-[#14532d] font-extrabold text-2xl">Compressed!</p>
                    <p className="text-[#15803d] text-sm font-bold mt-0.5 tracking-tight">
                      {result.originalSize - result.resultSize > 0 
                        ? `Your file is now ${Math.round(((result.originalSize - result.resultSize) / result.originalSize) * 100)}% smaller!` 
              ...
              <div className="grid grid-cols-2 gap-px bg-[#bbf7d0] overflow-hidden rounded-2xl outline outline-1 outline-[#bbf7d0]">
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                  <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">Original</p>
                  <p className="text-[#14532d] font-extrabold text-lg leading-none">{formatBytes(result.originalSize)}</p>
                </div>
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                   <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">Result</p>
          ...
          {status === 'error' && (
            <div className="mt-10 bg-[#fef2f2] border border-[#fee2e2] rounded-3xl p-8 flex items-start gap-3 transition-all duration-500">
              ...
              <div className="text-left">
                <p className="text-[#7f1d1d] font-extrabold text-lg leading-none mb-1">Compression Error</p>
        ...
        {/* Note: In your HTML you had local links here, but React handles navigation differently. Instead of hardcoded a href indexing, I used React Router's Link below if needed. */}
      </div>
    </div>
  );
};
