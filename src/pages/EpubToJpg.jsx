...
const EpubToJpg = () => {
    ...
    const API_URL = 'https://v2.convertapi.com/convert/epub/to/jpg?Secret=' + API_KEY;
    ...
    const addFiles = (files) => {
        const newFiles = Array.from(files)
            .filter(f => {
                const ext = f.name.toLowerCase().endsWith('.epub');
        ...
        <div className="min-h-screen bg-[#fbfcfe] pt-32 pb-16 px-4">
            <div className="max-w-4xl mx-auto text-center">
                ...
                <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                    <h1 className="text-4xl sm:text-5xl font-black text-[#1E2A5E] mb-4 tracking-tight">EPUB to JPG</h1>
                    <p className="text-gray-500 text-lg font-medium opacity-80 max-w-2xl mx-auto">
                        High-fidelity book synthesis. Convert EPUB documents into optimized JPG gallery assets in seconds.
                    </p>
                </header>
                        ...
                        <div className="p-10 sm:p-14 bg-gray-50/30">
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-[#1E2A5E] font-black text-lg italic mb-1 uppercase tracking-tight">Select EPUB Files</p>
                                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest leading-none mt-1 opacity-60">eBook Ready · Multi-File</p>
                                </div>
                                <input id="fileInput" type="file" accept=".epub" multiple className="hidden" onChange={(e) => addFiles(e.target.files)} />
                            </div>
                            ...
                            <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                                <div className="flex items-start gap-4">
                                    ...
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Multi-Page eBook Parsing</p>
                                </div>
                                <div className="flex items-start gap-4">
                                    <div className="w-6 h-6 rounded-full bg-green-50 text-green-500 flex items-center justify-center flex-shrink-0 shadow-sm"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" /></svg></div>
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">High-Res Layout Synthesis</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white">
                            ...
                            {selectedFiles.length === 0 ? (
                                <div className="text-center py-20 opacity-20">
                                    <p className="text-[#1E2A5E] font-black uppercase text-[10px] tracking-[5px]">Waiting for eBooks</p>
                                </div>
                            ) : (
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedFiles.map((item) => (
                                        <div key={item.id} className="bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group animate-in slide-in-from-right-2 duration-300">
                                            <div className="w-12 h-12 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-[#4F6EF7] shadow-sm font-black text-[9px] uppercase tracking-tighter shrink-0 text-center text-[#1E2A5E]">EPUB</div>
                            ...
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[#1E2A5E] font-black text-sm uppercase tracking-[5px]">
                                        {currentFile ? `Synthesizing: ${currentFile}` : 'PARSING EBOOK NODES...'}
                                    </p>
                                    <p className="text-gray-400 text-[10px] mt-2 uppercase font-black tracking-widest leading-none opacity-50">Mapping Book Layouts to Optimized JPG Buffers</p>
                                </div>
                            </div>
                ...
                <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                    {[
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.381z" /></svg>, title: "Elite eBook Engine", text: "Global cloud nodes ensure your EPUB documents are synthesized into optimized JPG assets in seconds without detail loss." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.9L9.03 9.069a1.888 1.888 0 010 3.268L2.166 16.5A2 2 0 010 14.564V6.836c0-1.104.97-1.936 2.166-1.936zM11 6a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1h-6a1 1 0 01-1-1V6z" /></svg>, title: "Batch Logic", text: "Easily upload multiple EPUB books at once. Our system handles massive document batch volumes with sequential cloud threads." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" /></svg>, title: "Secure Book Stream", text: "TLS 1.3 encryption ensures your eBooks are isolated and purged immediately post-synthesis." }
                    ].map((feature, i) => (
