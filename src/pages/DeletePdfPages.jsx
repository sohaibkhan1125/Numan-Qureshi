...
import { deletePdfPages } from '../services/api';
...
const DeletePdfPages = () => {
  ...
  const handleDeletePages = async () => {
    ...
    if (!pagesToDelete.trim()) {
      setErrorMsg('Please enter the page numbers or ranges to delete.');
      setStatus('error');
      return;
    }

    // Basic validation for page range format (e.g., 1,3,5-7)
    const pagePattern = /^[0-9,\-\s]+$/;
    if (!pagePattern.test(pagesToDelete)) {
      setErrorMsg('Invalid page format. Use numbers, commas, and hyphens (e.g., 1,3,5-7).');
      setStatus('error');
      return;
    }
    ...
    try {
      const fileResult = await deletePdfPages(selectedFile, pagesToDelete);
      ...
      setTimeout(() => {
        setResult({
          ...
          filename: fileResult.FileName || selectedFile.name.replace('.pdf', '_modified.pdf')
        });
        setStatus('success');
      ...
      clearInterval(progressTimer);
      setErrorMsg(err.message || 'An error occurred while deleting pages. Please check the page range.');
      setStatus('error');
    }
  };
    ...
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">Delete PDF Pages</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Easily remove specific pages from your PDF document</p>
        </header>
        ...
        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)]">
          ...
          <div className="mt-14 h-px bg-slate-100 relative">
            <span className="absolute left-1/2 -top-3 -translate-x-1/2 bg-white px-4 text-slate-300 text-[10px] font-extrabold uppercase tracking-widest">Pages to Delete</span>
          </div>
          ...
          <div className="mt-10 max-w-lg mx-auto">
            <input 
              ...
              className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 focus:border-[#4F6EF7] focus:ring-4 focus:ring-blue-100 outline-none transition-all text-[#1E2A5E] font-bold placeholder:text-slate-300"
            />
            <p className="text-slate-400 text-xs font-medium mt-3">Separate page numbers with commas or use hyphens for ranges.</p>
          </div>
          ...
          {status === 'processing' && (
            <div ref={progressRef} className="mt-10 p-8 bg-slate-50 border border-slate-100 rounded-[2rem] text-center transition-all duration-500">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <p className="text-[#1E2A5E] font-extrabold text-sm tracking-widest uppercase">REMOVING PAGES...</p>
              </div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div className="text-left">
                    <p className="text-[#14532d] font-extrabold text-2xl">Processed!</p>
                    <p className="text-[#15803d] text-sm font-bold mt-0.5 tracking-tight">Your PDF has been updated successfully.</p>
                  </div>
                </div>
          ...
          {status === 'error' && (
            <div className="mt-10 bg-[#fef2f2] border border-[#fee2e2] rounded-3xl p-8 flex items-start gap-3 transition-all duration-500">
              ...
              <div className="text-left">
                <p className="text-[#7f1d1d] font-extrabold text-lg leading-none mb-1">Error</p>
