...
const ParagraphWriter = () => {
  ...
  // Exact same ChatGPT API endpoint reused
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('https://chatgpt-api8.p.rapidapi.com/', {
        ...
        body: JSON.stringify([
          {
            content: "You are an expert content writer. Generate a single, well-structured, engaging, and highly professional paragraph based on the user's topic or instructions. Provide ONLY the requested paragraph, with no conversational filler like 'Here is your paragraph:'.",
            role: 'system',
          },
          {
            content: inputText,
      ...
      console.error(err);
      setError(err.message || 'Failed to generate paragraph.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#fbfcfe] pt-32 pb-16 px-4">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
          <h1 className="text-4xl sm:text-5xl font-black text-[#1E2A5E] mb-4 tracking-tight">Paragraph Writer</h1>
          <p className="text-gray-500 text-lg font-medium opacity-80 max-w-2xl mx-auto">
            Generate impactful, perfectly structured paragraphs for emails, essays, or articles in seconds using our AI engine.
          </p>
        </header>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(234,179,8,0.12)] border border-gray-100 overflow-hidden flex-1 flex flex-col">
              <div className="p-8 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  <label className={sectionLabel + ' mb-0'}>Topic & Instructions</label>
                  {inputText && !isLoading && (
                    <button
                      type="button"
                ...
                <textarea
                  ...
                  placeholder="What would you like the paragraph to be about? Provide details, tone, or key points..."
          ...
          <div className="space-y-4 flex flex-col">
            <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(234,179,8,0.12)] border border-gray-100 overflow-hidden flex-1 flex flex-col min-h-[350px]">
              <div className="p-8 flex-1 flex flex-col relative">
                <div className="flex items-center justify-between mb-4">
                  <label className={sectionLabel + ' mb-0'}>Generated Paragraph</label>
                  {outputText && (
                    <button
                      type="button"
                      onClick={handleCopy}
                ...
                {isLoading ? (
                  <div className="flex-1 flex flex-col items-center justify-center opacity-40">
                    ...
                    <p className="text-[10px] uppercase tracking-widest font-bold text-[#1E2A5E]">Drafting paragraph active...</p>
                  </div>
                ) : error ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center opacity-20 transition-opacity duration-300">
                    ...
                    <p className="font-black uppercase tracking-[5px] text-[10px]">Awaiting Instructions</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {[
            {
              t: 'Precise Focus',
              d: 'Generates exactly what you need without fluff—delivering a single, powerful paragraph that dives right into the heart of the topic.',
              icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" /></svg>,
            },
            {
              t: 'Flawless Flow',
              d: 'Ensures topic sentences, supporting evidence, and concluding thoughts interlock seamlessly for high-impact readability.',
              icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>,
            },
            {
              t: 'Adaptable Tone',
              d: 'Adjusts writing style automatically based on your instructions—from corporate professional to passionately creative.',
              icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>,
            },
          ].map(x => (
