...
const BmpToWebp = () => {
    ...
    const API_URL = 'https://v2.convertapi.com/convert/bmp/to/webp?Secret=' + API_KEY;
    ...
    const b64ToBlob = (b64, mime = 'image/webp') => {
        ...
        for (let i = 0; i < bytes.length; i += 512) {
            ...
            for (let j = 0; j < chunk.length; j++) {
                byteNumbers[j] = chunk.charCodeAt(0);
            }
    ...
    const addFiles = (files) => {
        const newFiles = Array.from(files)
            .filter(f => f.name.toLowerCase().endsWith('.bmp'))
        ...
        <div className="min-h-screen bg-[#fbfcfe] pt-32 pb-16 px-4">
            <div className="max-w-4xl mx-auto text-center">
                ...
                <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                    <h1 className="text-4xl sm:text-5xl font-black text-[#1E2A5E] mb-4 tracking-tight">BMP to WEBP</h1>
                    <p className="text-gray-500 text-lg font-medium opacity-80 max-w-2xl mx-auto">
                        High-efficiency modern synthesis. Convert bulky bitmap files into optimized, high-fidelity WEBP gallery assets in seconds.
                    </p>
                </header>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)] border border-gray-100 overflow-hidden text-left transition-all duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div className="p-10 sm:p-14 bg-gray-50/30">
                            ...
                            <div 
                                onDragOver={(e) => { e.preventDefault(); e.currentTarget.classList.add('border-[#4F6EF7]', 'bg-blue-50/20'); }}
                                onDragLeave={(e) => { e.currentTarget.classList.remove('border-[#4F6EF7]', 'bg-blue-50/20'); }}
                                onDrop={(e) => { 
                                    e.preventDefault(); 
                                    e.currentTarget.classList.remove('border-[#4F6EF7]', 'bg-blue-50/20'); 
                                    addFiles(e.dataTransfer.files); 
                                }}
                                onClick={() => document.getElementById('fileInput').click()}
                                className="border-2 border-dashed border-gray-100 rounded-[2.5rem] flex flex-col items-center justify-center p-14 cursor-pointer transition-all bg-white group relative hover:border-[#4F6EF7]"
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-[#1E2A5E] font-black text-lg italic mb-1">Select BMP Images</p>
                                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest leading-none mt-1">Cloud Ready · Multiple Assets</p>
                                </div>
                                <input id="fileInput" type="file" accept=".bmp" multiple className="hidden" onChange={(e) => addFiles(e.target.files)} />
                            </div>
                            ...
                            <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                                ...
                                <div className="flex items-start gap-4">
                                    <div className="w-6 h-6 rounded-full bg-green-50 text-green-500 flex items-center justify-center flex-shrink-0 shadow-sm"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" /></svg></div>
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">High-Compression Buffer Parsing</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white">
                            ...
                            {selectedFiles.length === 0 ? (
                                <div className="text-center py-20 opacity-20">
                                    <p className="text-[10px] font-black uppercase tracking-[5px]">Waiting for BMP Assets</p>
                                </div>
                            ) : (
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedFiles.map((item) => (
                                        <div key={item.id} className="bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group animate-in slide-in-from-right-2">
                                            <div className="w-12 h-12 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-[#4F6EF7] shadow-sm font-black text-[9px] uppercase tracking-tighter shrink-0">BMP</div>
                    ...
                    {isConverting && (
                        <div className="p-12 bg-gray-50 border-t border-gray-100 text-center animate-in fade-in duration-500">
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[#1E2A5E] font-black text-sm uppercase tracking-[5px]">ENCODING MODERN STREAM...</p>
                                    <p className="text-gray-400 text-[10px] mt-2 uppercase font-black tracking-widest leading-none opacity-50">Mapping Bitmap Objects to Efficient WEBP Nodes</p>
                                </div>
                            </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white border-t border-gray-100 text-left animate-in fade-in duration-700">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <p className="text-gray-400 text-[10px] font-black mt-2 uppercase tracking-widest opacity-60">High-Resolution WEBP Assets Generated</p>
                                </div>
                                ...
                                {generatedFiles.map((f, i) => (
                                    <div key={i} className="group bg-gray-50/50 border border-gray-100 rounded-2xl overflow-hidden relative animate-in zoom-in-95 duration-500 shadow-sm hover:shadow-xl transition-all">
                                        <div className="aspect-square overflow-hidden relative">
                                            <img src={f.url} alt={f.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
