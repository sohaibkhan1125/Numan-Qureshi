...
const DwfToWebp = () => {
    ...
    const API_URL = 'https://v2.convertapi.com/convert/dwf/to/webp?Secret=' + API_KEY;
    const MAX_FILE_SIZE = 50 * 1024 * 1024; // 50MB Limit

    // ── STATE ───────────────────────────────────────────
    const [selectedFiles, setSelectedFiles] = useState([]); // array of { file, id }
    const [generatedFiles, setGeneratedFiles] = useState([]); // { name, blob, url }
    ...
    const b64ToBlob = async (b64, mime = 'image/webp') => {
        // High-performance async blob synthesis via data-URL fetch
        // This offloads the decoding from the main thread and uses browser's native C++ logic
        try {
            const response = await fetch(`data:${mime};base64,${b64}`);
            return await response.blob();
        } catch (e) {
            // Fallback for edge cases using high-performance ArrayBuffer
    ...
    const addFiles = (files) => {
        const newFiles = Array.from(files)
            .filter(f => {
                const ext = f.name.toLowerCase().endsWith('.dwf');
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            // Sequential processing avoids memory spikes and main-thread exhaustion
            for (const item of selectedFiles) {
                setCurrentFile(item.file.name);
                
                // 1. Optimized File Read
                const b64 = await readB64(item.file);
                
                // 2. High-performance API Transfer
        ...
        <div className="min-h-screen bg-[#fbfcfe] pt-32 pb-16 px-4">
            <div className="max-w-4xl mx-auto text-center">
                ...
                <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                    <h1 className="text-4xl sm:text-5xl font-black text-[#1E2A5E] mb-4 tracking-tight">DWF to WEBP</h1>
                    <p className="text-gray-500 text-lg font-medium opacity-80 max-w-2xl mx-auto">
                        High-fidelity CAD synthesis. Convert Autodesk DWF drawings into optimized, modern WEBP images in seconds.
                    </p>
                </header>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)] border border-gray-100 overflow-hidden text-left transition-all duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div className="p-10 sm:p-14 bg-gray-50/30">
                            <label className="block text-[10px] font-black uppercase tracking-[4px] text-gray-300 mb-8">Step 1: Drawing Loading</label>
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-[#1E2A5E] font-black text-lg italic mb-1 uppercase tracking-tight">Select DWF Files</p>
                                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest leading-none mt-1 opacity-60">CAD Ready · Multi-Sheet</p>
                                </div>
                                <input id="fileInput" type="file" accept=".dwf" multiple className="hidden" onChange={(e) => addFiles(e.target.files)} />
                            </div>
                            ...
                            <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                                <div className="flex items-start gap-4">
                                    ...
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Modern WEBP Encoding Logic</p>
                                </div>
                                <div className="flex items-start gap-4">
                                    <div className="w-6 h-6 rounded-full bg-green-50 text-green-500 flex items-center justify-center flex-shrink-0 shadow-sm"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" /></svg></div>
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">High-Fidelity Layout Synthesis</p>
                                </div>
                            </div>
                        </div>

                        {/* Right: Synthesis Config */}
                        <div className="p-10 sm:p-14 bg-white">
                            ...
                            {selectedFiles.length === 0 ? (
                                <div className="text-center py-20 opacity-20">
                                    <p className="text-[#1E2A5E] font-black uppercase text-[10px] tracking-[5px]">Waiting for CAD Files</p>
                                </div>
                            ) : (
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedFiles.map((item) => (
                                        <div key={item.id} className="bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group animate-in slide-in-from-right-2 duration-300">
                                            <div className="w-12 h-12 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-[#4F6EF7] shadow-sm font-black text-[9px] uppercase tracking-tighter shrink-0 text-center">DWF</div>
                            ...
                            <div className="mt-10 pt-8 border-t border-gray-50">
                                ...
                                >
                                    {isConverting ? 'Synthesizing...' : 'Convert to WEBP'}
                                </button>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[#1E2A5E] font-black text-sm uppercase tracking-[5px]">
                                        {currentFile ? `Synthesizing: ${currentFile}` : 'PARSING CAD NODES...'}
                                    </p>
                                    <p className="text-gray-400 text-[10px] mt-2 uppercase font-black tracking-widest leading-none opacity-50">Mapping CAD Vertices to Optimized WEBP Objects</p>
                                </div>
                            </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white border-t border-gray-100 text-left animate-in fade-in duration-700">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <p className="text-gray-400 text-[10px] font-black mt-2 uppercase tracking-widest opacity-60">High-Resolution WEBP Pages Generated</p>
                                </div>
                                ...
                                {generatedFiles.map((f, i) => (
                                    <div key={i} className="group bg-gray-50/50 border border-gray-100 rounded-2xl overflow-hidden relative animate-in zoom-in-95 duration-500 shadow-sm hover:shadow-xl transition-all">
                                        <div className="aspect-square bg-white relative p-4 flex items-center justify-center overflow-hidden">
                                            <img src={f.url} alt={f.name} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-700" />
                                            <div className="absolute inset-0 bg-[#1E2A5E]/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                 <a href={f.url} download={f.name} className="p-4 bg-white text-[#4F6EF7] rounded-full shadow-lg hover:scale-110 transition-transform">
                ...
                <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                    {[
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.381z" /></svg>, title: "Elite CAD Engine", text: "Global cloud nodes ensure your Autodesk DWF drawings are synthesized into optimized WEBP assets in seconds without detail loss." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.9L9.03 9.069a1.888 1.888 0 010 3.268L2.166 16.5A2 2 0 010 14.564V6.836c0-1.104.97-1.936 2.166-1.936zM11 6a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1h-6a1 1 0 01-1-1V6z" /></svg>, title: "Batch Logic", text: "Easily upload multiple DWF files at once. Our system handles massive CAD batch volumes with cloud-isolated threads." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" /></svg>, title: "Secure CAD Stream", text: "TLS 1.3 encryption ensures your CAD drawings are isolated and purged immediately post-synthesis." }
                    ].map((feature, i) => (
