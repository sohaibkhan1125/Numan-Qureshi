...
import { convertTiffToPdf } from '../services/api';
...
const TiffToPdf = () => {
    ...
    const validateFile = (file) => {
        ...
        const isTiff = validTypes.includes(file.type) || /\.(tiff?)$/i.test(file.name);
        if (!isTiff) {
            setError('Please upload a valid TIFF/TIF image file.');
            return false;
        }
    ...
    const handleConvert = async () => {
        ...
        try {
            const data = await convertTiffToPdf(file);
            ...
            clearInterval(interval);
            setError(err.message || 'Failed to convert TIFF. Please try again.');
            setStatus('error');
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 pt-28 pb-20 px-4">
            <div className="container mx-auto max-w-4xl">
                <div className="text-center mb-12">
                    ...
                    >
                        TIFF to <span className="text-blue-500">PDF</span>
                    </motion.h1>
                    <motion.p 
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        Convert complex TIFF raster graphics into simple, universally viewable PDF documents instantly.
                    </motion.p>
                </div>
                    ...
                    <AnimatePresence mode="wait">
                                ...
                                {!file ? (
                                    ...
                                    >
                                        ...
                                        <div className="text-center">
                                            <h3 className="text-2xl font-black text-slate-800 mb-3">Drop TIFF image here</h3>
                                            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Supports single & multi-page TIFF layers</p>
                                        </div>
                                    </div>
                                ) : (
                        ...
                        {status === 'processing' && (
                            ...
                            >
                                ...
                                <h3 className="text-2xl font-black text-slate-800 mb-2">Processing Layers</h3>
                                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                                    <Loader2 className="w-4 h-4 animate-spin text-blue-500" />
                                    Converting TIFF frames to PDF vector structure...
                                </p>
                            </motion.div>
                        )}
                        ...
                        {status === 'success' && (
                            ...
                            >
                                ...
                                <p className="text-slate-500 font-medium text-center mb-12 max-w-md">Your TIFF image file has been successfully formatted into a PDF document.</p>
                ...
                <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
                    {[
                        { title: 'Quality Retained', desc: 'Conversions compress the structural padding without ever destroying the intrinsic layer detail.' },
                        { title: 'Multi-Page Tiff', desc: 'Handles large complex architecture and maps retaining any multi-pagination natively.' },
                        { title: 'Encrypted Safely', desc: 'End to End HTTPS standard mapping securely flushes all raster assets post-download.' }
                    ].map((feature, i) => (
