...
const FacebookDownloader = () => {
  ...
  const startDownload = async () => {
    if (!url.trim() || (!url.includes('facebook.com') && !url.includes('fb.watch'))) {
    ...
    setDownloadLinks([]);
    setVideoInfo(null);
    setProgressMsg('Fetching Facebook information...');

    try {
      const encodedUrl = encodeURIComponent(url);
      const apiPath = `/api/v1/facebook-media/info?url=${encodedUrl}`;
    ...
    <div className="bg-slate-50 min-h-screen pt-24 pb-12">
      ...
      <div className="container mx-auto px-4 md:px-6 mb-8 max-w-4xl">
        <nav className="flex items-center gap-2 text-sm text-slate-400 font-medium">
          ...
          <span>›</span>
          <span className="text-slate-600">Facebook Downloader</span>
        </nav>
      </div>

      <div className="container mx-auto px-4 md:px-6 max-w-4xl">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-blue-600/10 text-blue-600 mb-6 group-hover:scale-110 transition-transform shadow-sm">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-slate-800 mb-4 tracking-tight">Facebook Downloader</h1>
          <p className="text-lg text-slate-500 font-medium max-w-xl mx-auto">
            Download Facebook videos easily in High Definition or Standard Definition.
          </p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-10 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
          ...
          {!videoInfo && !isLoading && (
            <div className="flex flex-col gap-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">Facebook Video URL</label>
              ...
              <button
                ...
                className="w-full bg-[#1877f2] hover:bg-blue-700 text-white font-black text-lg py-4 rounded-xl transition-all shadow-lg shadow-blue-500/25 mt-4 active:scale-[0.98]"
              >
                Fetch Media
              </button>
            </div>
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <svg className="animate-spin -ml-1 mr-3 h-12 w-12 text-[#1877f2] mb-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            {videoInfo && !isLoading && (
              ...
              >
                <div className="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center gap-4 text-center shadow-sm mb-8 relative overflow-hidden">
                  ...
                  <div className="pt-2">
                    <h3 className="font-black text-slate-800 text-lg leading-snug px-4 line-clamp-2">{videoInfo.title || "Facebook Video"}</h3>
                    ...
                    <div className="flex flex-col gap-3 w-full">
                      {downloadLinks.map((linkObj, index) => (
                        <a
                           ...
                           target="_blank"
                           rel="noreferrer"
                           className="flex items-center justify-between bg-white border border-slate-200 p-4 rounded-xl hover:border-[#1877f2] hover:shadow-md transition-all active:scale-[0.99] no-underline decoration-none group"
                        >
                           <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                <span className="block font-bold text-slate-800 uppercase text-sm tracking-wider">
                                  {linkObj.type || 'Download'} {linkObj.resolution ? `(${linkObj.resolution})` : ''}
                                </span>
                                <span className="block text-slate-500 text-xs font-medium">Facebook Media</span>
                              </div>
                           </div>
                           <div className="bg-slate-50 text-[#1877f2] font-bold p-2 px-4 rounded-lg group-hover:bg-[#1877f2] group-hover:text-white transition-colors text-sm">
                             Download
                           </div>
                        </a>
                      ))}
