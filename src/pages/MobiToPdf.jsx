...
import { convertMobiToPdf } from '../services/api';
...
const MobiToPdf = () => {
  ...
  const handleFileSelected = (file) => {
    ...
    if (!fileName.endsWith('.mobi')) {
      alert('Please select a valid MOBI file.');
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      const fileResult = await convertMobiToPdf(selectedFile);
      ...
      setTimeout(() => {
        setResult({
          ...
          filename: fileResult.FileName || selectedFile.name.replace(/\.mobi$/i, '.pdf')
        });
        setStatus('success');
  ...
  return (
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans text-left">
      <style>{`
        ...
        .mobi-badge-gradient { background: linear-gradient(135deg, #3b82f6, #1d4ed8); }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">MOBI to PDF</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Convert your MOBI ebooks to PDF format instantly</p>
        </header>
        ...
        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(59,130,246,0.12)]">
          ...
          {!selectedFile && (
            ...
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl mobi-badge-gradient flex items-center justify-center text-white shadow-xl group-hover:scale-105 transition-transform duration-300">
                <svg className="w-10 h-10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
              </div>
              <div className="text-center">
                <p className="text-[#1E2A5E] font-bold text-xl mb-2">Drag & drop MOBI file here</p>
                <p className="text-slate-400 font-medium">or <span className="text-[#3b82f6] font-semibold">click to browse</span></p>
              </div>
              <input ref={fileInputRef} type="file" accept=".mobi" className="hidden" onChange={handleFileChange} />
            </div>
          )}
          ...
          {selectedFile && (
            <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between transition-all duration-500">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-2xl mobi-badge-gradient flex items-center justify-center text-white text-[10px] font-bold shadow-sm">MOBI</div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              ...
              <div className="grid grid-cols-2 gap-px bg-[#bbf7d0] overflow-hidden rounded-2xl outline outline-1 outline-[#bbf7d0]">
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                  <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">MOBI Size</p>
