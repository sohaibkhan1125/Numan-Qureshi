...
const AiToWebp = () => {
    ...
    const API_URL = 'https://v2.convertapi.com/convert/ai/to/webp?Secret=' + API_KEY;
    ...
    const b64ToBlob = (b64, mime = 'image/webp') => {
    ...
    const addFiles = (files) => {
        const newFiles = Array.from(files).filter(f => f.name.toLowerCase().endsWith('.ai')).map(f => ({
            file: f,
    ...
    return (
        <div className="min-h-screen bg-[#fbfcfe]">
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                .vector-badge { background: linear-gradient(135deg, #FF9A00, #E65100); }
            ...
            <div className="max-w-4xl mx-auto px-4 py-16 text-center">
                <header className="mb-12">
                    <h1 className="text-4xl sm:text-5xl font-black mb-4 tracking-tight text-[#1E2A5E]">AI to WEBP</h1>
                    <p className="text-[#1E2A5E]/60 text-lg font-bold max-w-2xl mx-auto uppercase tracking-wider">
                        Next-gen vector synthesis. Convert Adobe Illustrator files into highly optimized WEBP assets with alpha transparency.
                    </p>
                </header>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)] border border-gray-100 transition-all duration-500 overflow-hidden text-left">
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        ...
                        <div className="p-10 sm:p-14 bg-gray-50/30">
                            <label className="block text-[10px] font-black uppercase tracking-[4px] text-gray-300 mb-8">Step 1: Vector Loading</label>
                            ...
                            >
                                <div className="w-16 h-16 rounded-3xl vector-badge flex items-center justify-center text-white mb-6 shadow-lg transition-transform group-hover:scale-105">
                                    <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-6h2v6zm4 0h-2V7h2v10z"/></svg>
                                </div>
                                <div className="text-center">
                                    <p className="text-[#1E2A5E] font-black text-lg italic mb-1 uppercase tracking-tight">Select AI Files</p>
                                    <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest leading-none mt-1">Illustrator Library · Multi-Artboard</p>
                                </div>
                                <input ref={fileInputRef} type="file" accept=".ai" multiple className="hidden" onChange={(e) => { addFiles(e.target.files); e.target.value = ''; }} />
                            </div>
                            ...
                            <div className="mt-12 space-y-6 pt-10 text-left border-t border-gray-100">
                                <div className="flex items-start gap-4">
                                    ...
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Full Alpha Channel Injection</p>
                                </div>
                                <div className="flex items-start gap-4">
                                    <div className="w-6 h-6 rounded-full bg-green-50 text-green-500 flex items-center justify-center flex-shrink-0 shadow-sm"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" /></svg></div>
                                    <p className="text-gray-400 text-[11px] font-black uppercase tracking-[2px] leading-relaxed">Artboard-Isolated Synthesis</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white">
                            <label className="block text-[10px] font-black uppercase tracking-[4px] text-gray-300 mb-8">Step 2: Synthesis Management</label>
                            
                            {selectedFiles.length === 0 ? (
                                <div className="text-center py-20 opacity-20">
                                    <p className="text-[#1E2A5E] font-black uppercase text-[10px] tracking-[5px]">Waiting for Vectors</p>
                                </div>
                            ) : (
                                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedFiles.map((item) => (
                                        <div key={item.id} className="img-card bg-gray-50/50 border border-gray-100 rounded-3xl p-5 flex items-center gap-5 group animate-in slide-in-from-right-2 duration-300">
                                            <div className="w-12 h-12 rounded-2xl bg-white border border-gray-100 flex items-center justify-center text-orange-500 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] font-black text-[9px] uppercase tracking-tighter shrink-0 text-[#1E2A5E]">AI</div>
                    ...
                    {isConverting && (
                        <div className="p-12 bg-gray-50 border-t border-gray-100 text-center animate-in fade-in duration-500">
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div>
                                    <p className="text-[#1E2A5E] font-black text-sm uppercase tracking-[5px]">PARSING VECTOR NODES...</p>
                                    <p className="text-gray-400 text-[10px] mt-2 uppercase font-bold tracking-widest leading-none">Mapping Illustration Paths to Transparent WEBP Buffers</p>
                                </div>
                            </div>
                        ...
                        <div className="p-10 sm:p-14 bg-white border-t border-gray-100 text-left">
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <p className="text-gray-400 text-[10px] font-bold mt-2 uppercase tracking-widest opacity-60">Highly Optimized Transparent Assets Generated</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 pb-4">
                                {generatedImages.map((img, i) => (
                                    <div key={i} className="img-card bg-gray-50/50 border border-gray-100 rounded-2xl overflow-hidden group animate-in zoom-in-95 duration-500 shadow-sm hover:shadow-xl">
                                        <div className="aspect-[3/2] overflow-hidden relative transparent-bg">
                                            <img src={img.url} alt={img.name} className="w-full h-full object-contain p-4" />
                ...
                <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.381z" /></svg>, title: "Elite Vector Engine", text: "Global cloud nodes ensure your Illustrator vectors are synthesized into highly optimized WEBP assets." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.9L9.03 9.069a1.888 1.888 0 010 3.268L2.166 16.5A2 2 0 010 14.564V6.836c0-1.104.97-1.936 2.166-1.936zM11 6a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1h-6a1 1 0 01-1-1V6z" /></svg>, title: "Sequential Artboards", text: "Easily upload multiple AI files at once. Our system handles massive vector volumes." },
                        { icon: <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" /></svg>, title: "Secure Vector Stream", text: "TLS 1.3 encryption ensures your vector files are isolated and purged immediately post-synthesis." }
                    ].map((feature, i) => (
