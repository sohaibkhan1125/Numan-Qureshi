...
import { convertOfficeToPdf } from '../services/api';
...
const OfficeToPdf = () => {
  ...
  const handleFileSelected = (file) => {
    ...
    if (!isValid) {
      alert('Please select a valid Microsoft Office file (.doc, .docx, .xls, .xlsx, .ppt, .pptx).');
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      const fileResult = await convertOfficeToPdf(selectedFile);
      ...
      setTimeout(() => {
        setResult({
          ...
          filename: fileResult.FileName || selectedFile.name.replace(/\.[^/.]+$/, "") + ".pdf"
        });
        setStatus('success');
  ...
  return (
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans text-left">
      <style>{`
        ...
        .shimmer-active {
          background: linear-gradient(90deg, #3b82f6 25%, #10b981 50%, #3b82f6 75%);
          background-size: 400px 100%;
          animation: shimmer 1.4s infinite;
        }

        .office-badge-gradient { background: linear-gradient(135deg, #2b579a, #1d4ed8); }
        .pdf-badge-gradient { background: linear-gradient(135deg, #10b981, #059669); }
      `}</style>
      
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">Office to PDF</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Convert Word, Excel, and PowerPoint files to PDF instantly</p>
        </header>

        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(59,130,246,0.12)]">
          ...
          {!selectedFile && (
            <div 
              ...
              className={`relative border-2 border-dashed border-slate-200 rounded-2xl md:rounded-3xl p-8 md:p-14 flex flex-col items-center justify-center gap-4 md:gap-6 cursor-pointer transition-all duration-300 group ${dragActive ? 'drag-active bg-blue-50/30 border-[#3b82f6]' : 'hover:border-[#3b82f6] hover:bg-blue-50/30'}`}
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl office-badge-gradient flex items-center justify-center text-white shadow-xl group-hover:scale-105 transition-transform duration-300">
                <svg className="w-10 h-10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                   <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <div className="text-center">
                <p className="text-[#1E2A5E] font-bold text-xl mb-2">Drag & drop Office file here</p>
                <p className="text-slate-400 font-medium">supports .docx, .xlsx, .pptx and more</p>
                <p className="text-slate-400 font-medium mt-1">or <span className="text-[#3b82f6] font-semibold">click to browse</span></p>
              </div>
              <input ref={fileInputRef} type="file" accept=".doc,.docx,.xls,.xlsx,.ppt,.pptx" className="hidden" onChange={handleFileChange} />
            </div>
          )}
          ...
          {selectedFile && (
            <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between transition-all duration-500">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-2xl office-badge-gradient flex items-center justify-center text-white text-[10px] font-bold shadow-sm">DOC/XLS</div>
          ...
          <div className="mt-14">
            <button 
              ...
              style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)', boxShadow: '0 10px 30px -5px rgba(59, 130, 246, 0.4)' }}
            >
              Convert to PDF
            </button>
          </div>
          ...
          {status === 'processing' && (
            <div ref={progressRef} className="mt-10 p-8 bg-slate-50 border border-slate-100 rounded-[2rem] text-center transition-all duration-500">
              <div className="flex items-center justify-center gap-3 mb-4">
                <div className="w-6 h-6 animate-spin rounded-full border-2" style={{ borderColor: '#E2E8F0', borderTopColor: '#3b82f6' }}></div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              ...
              <div className="grid grid-cols-2 gap-px bg-[#bbf7d0] overflow-hidden rounded-2xl outline outline-1 outline-[#bbf7d0]">
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                  <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">Office Size</p>
