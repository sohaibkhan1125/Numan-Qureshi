...
const JpgToPdf = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      const fileResult = await convertJpgToPdf(selectedFile);
      ...
      setTimeout(() => {
        setResult({
          ...
          filename: fileResult.FileName || selectedFile.name.replace(/\.(jpg|jpeg)$/i, '.pdf')
        });
        setStatus('success');
  ...
  return (
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans text-left">
      <style>{`
        ...
        .jpg-badge-gradient { background: linear-gradient(135deg, #22c55e, #10b981); }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">JPG to PDF</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Convert your JPG images to PDF format instantly</p>
        </header>
        ...
        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)]">
          ...
          {!selectedFile && (
            ...
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl jpg-badge-gradient flex items-center justify-center text-white shadow-xl group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="text-center">
                <p className="text-[#1E2A5E] font-bold text-xl mb-2">Drag & drop JPG/JPEG file here</p>
                <p className="text-slate-400 font-medium">or <span className="text-[#4F6EF7] font-semibold">click to browse</span></p>
              </div>
              <input ref={fileInputRef} type="file" accept=".jpg,.jpeg" className="hidden" onChange={handleFileChange} />
            </div>
          )}
          ...
          {selectedFile && (
            <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between transition-all duration-500">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 rounded-2xl jpg-badge-gradient flex items-center justify-center text-white text-[10px] font-bold shadow-sm">JPG</div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              ...
              <div className="grid grid-cols-2 gap-px bg-[#bbf7d0] overflow-hidden rounded-2xl outline outline-1 outline-[#bbf7d0]">
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                  <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">JPG Size</p>
