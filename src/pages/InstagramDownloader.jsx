...
const InstagramDownloader = () => {
  ...
  const startDownload = async () => {
    if (!url.trim() || !url.includes('instagram.com')) {
      setError('Please enter a valid Instagram URL (e.g. Reel, IGTV, Post)');
      return;
    }
    ...
    setDownloadLinks([]);
    setVideoInfo(null);
    setProgressMsg('Fetching Instagram information...');

    try {
      const encodedUrl = encodeURIComponent(url);
      const apiPath = `/api/v1/instagram-media/info?url=${encodedUrl}`;
    ...
    <div className="bg-slate-50 min-h-screen pt-24 pb-12">
      ...
      <div className="container mx-auto px-4 md:px-6 mb-8 max-w-4xl">
        <nav className="flex items-center gap-2 text-sm text-slate-400 font-medium">
          ...
          <span>›</span>
          <span className="text-slate-600">Instagram Downloader</span>
        </nav>
      </div>

      <div className="container mx-auto px-4 md:px-6 max-w-4xl">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-pink-500/10 text-pink-600 mb-6 group-hover:scale-110 transition-transform shadow-sm">
             <svg className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-slate-800 mb-4 tracking-tight">Instagram Downloader</h1>
          <p className="text-lg text-slate-500 font-medium max-w-xl mx-auto">
            Download Instagram Reels, IGTV, and Post videos directly to your device.
          </p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 md:p-10 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
          ...
          {!videoInfo && !isLoading && (
            <div className="flex flex-col gap-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 ml-1">Instagram Media URL</label>
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 hover:from-purple-600 hover:via-pink-600 hover:to-orange-500 text-white font-black text-lg py-4 rounded-xl transition-all shadow-lg shadow-pink-500/25 mt-4 active:scale-[0.98]"
              >
                Fetch Media
              </button>
            </div>
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <svg className="animate-spin -ml-1 mr-3 h-12 w-12 text-pink-500 mb-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            {videoInfo && !isLoading && (
              ...
              >
                <div className="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center gap-4 text-center shadow-sm mb-8 relative overflow-hidden">
                  {videoInfo.thumbnail ? (
                      <img src={videoInfo.thumbnail} alt={videoInfo.title} className="w-full h-48 object-cover rounded-xl shadow-sm" />
                  ) : (
                  ...
                  <div className="pt-2">
                    <h3 className="font-black text-slate-800 text-lg leading-snug px-4 line-clamp-2">{videoInfo.title || "Instagram Post"}</h3>
                    <p className="text-slate-500 text-sm font-medium mt-1">@{videoInfo.uploader} {videoInfo.duration && `• ${videoInfo.duration}`}</p>
                  </div>
                </div>
                    ...
                    <div className="flex flex-col gap-3 w-full">
                      {downloadLinks.map((linkObj, index) => (
                        <a
                           ...
                           target="_blank"
                           rel="noreferrer"
                           className="flex items-center justify-between bg-white border border-slate-200 p-4 rounded-xl hover:border-pink-500 hover:shadow-md transition-all active:scale-[0.99] no-underline decoration-none group"
                        >
                           <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${linkObj.type === 'image' ? 'bg-indigo-100 text-indigo-600' : 'bg-pink-100 text-pink-600'}`}>
                                {linkObj.type === 'audio' ? (
                                   ...
                                   <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h14a2 2 0 002-2V6a2 2 0 00-2-2H4a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                ) : (
                              ...
                              <div className="text-left">
                                ...
                                <span className="block text-slate-500 text-xs font-medium">Instagram Media</span>
                              </div>
                           </div>
                           <div className="bg-slate-50 text-pink-600 font-bold p-2 px-4 rounded-lg group-hover:bg-gradient-to-r group-hover:from-purple-500 group-hover:to-pink-500 group-hover:text-white transition-all duration-300 text-sm">
                             Download
                           </div>
                        </a>
                      ))}
