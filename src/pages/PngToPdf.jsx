...
import { Image as ImageIcon, Download, Upload, CheckCircle, AlertCircle, Loader2, FileCheck } from 'lucide-react';
import { convertPngToPdf } from '../services/api';
...
const PngToPdf = () => {
    ...
    const handleConvert = async () => {
        ...
        try {
            const data = await convertPngToPdf(file);
            ...
            clearInterval(interval);
            setError(err.message || 'Failed to convert PNG to PDF. Please try again.');
            setStatus('error');
        }
    };
        ...
        <div className="min-h-screen bg-slate-50 pt-28 pb-20 px-4">
            <div className="container mx-auto max-w-4xl">
                <div className="text-center mb-12">
                    ...
                    >
                        PNG to <span className="text-blue-600">PDF</span>
                    </motion.h1>
                    <motion.p 
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        Convert your high-fidelity PNG images into PDF documents while retaining transparency and perfect resolution.
                    </motion.p>
                </div>
                ...
                <motion.div
                    ...
                    className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-[0_20px_50px_rgba(37,99,235,0.05)] border border-blue-50 relative overflow-hidden"
                >
                    <AnimatePresence mode="wait">
                        {status === 'idle' && (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className={`w-20 h-20 rounded-3xl flex items-center justify-center transition-all duration-500 mb-8 ${isDragging ? 'bg-blue-600 text-white scale-110 rotate-12' : 'bg-blue-50 text-blue-500 group-hover:scale-110 group-hover:bg-blue-100 shadow-sm'}`}>
                                    ...
                                    {file ? (
                                        ...
                                        <div className="text-center">
                                            <h3 className="text-2xl font-black text-slate-800 mb-3">Drop PNG image here</h3>
                                            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Convert directly to PDF format</p>
                                        </div>
                                    )}
                                </div>
                        ...
                        {status === 'processing' && (
                            ...
                            >
                                ...
                                <h3 className="text-2xl font-black text-slate-800 mb-2">Converting Image</h3>
                                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                                    <Loader2 className="w-4 h-4 animate-spin text-blue-500" />
                                    Optimizing layout into PDF document...
                                </p>
                            </motion.div>
                        )}
                        ...
                        {status === 'success' && (
                            ...
                            >
                                ...
                                <p className="text-slate-500 font-medium text-center mb-12 max-w-md">Your PNG image was successfully inserted into a standard PDF document.</p>
                                ...
                                <div className="w-full bg-blue-50/50 rounded-3xl p-6 mb-12 border border-blue-100 flex items-center justify-between group hover:bg-blue-50 transition-colors">
                                    ...
                                    <a 
                                        href={result?.Url}
                                        download
                                        className="p-4 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-600/30 hover:bg-blue-700 hover:scale-110 active:scale-95 transition-all"
                                    >
                ...
                <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
                    {[
                        { title: 'Crisp Resolution', desc: 'No quality is lost when translating transparent PNGs into standard portable document formats.' },
                        { title: 'High Speed', desc: 'Process images at lightning speeds utilizing modern global edge networks.' },
                        { title: 'Secure Handling', desc: 'All files are automatically deleted from server traces directly after the conversion step.' }
                    ].map((feature, i) => (
                        <div key={i} className="p-4">
                            <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-blue-600 mb-6 shadow-sm border border-blue-100 mx-auto md:mx-0">
