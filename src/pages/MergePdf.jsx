...
const MergePdf = () => {
  ...
  const handleFilesSelected = (files) => {
    const validPdfs = files.filter(file => file.name.toLowerCase().endsWith('.pdf'));
    ...
    // Add new files to the existing selection, avoiding duplicates by name and size
    setSelectedFiles(prev => {
      const newFiles = validPdfs.filter(vf => !prev.some(pf => pf.name === vf.name && pf.size === vf.size));
  ...
  const handleMerge = async () => {
    ...
    try {
      const fileResult = await mergePdfs(selectedFiles);
      ...
      setTimeout(() => {
        setResult({
          blob,
          originalTotalSize: selectedFiles.reduce((acc, f) => acc + f.size, 0),
          resultSize: blob.size,
          downloadUrl: URL.createObjectURL(blob),
          filename: fileResult.FileName || 'merged.pdf'
        });
        setStatus('success');
  ...
  return (
    <div className="bg-[#f8fafc]/50 min-h-screen font-sans text-left">
      <style>{`
        ...
        .merge-badge-gradient { background: linear-gradient(135deg, #3b82f6, #2563eb); }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16 text-center">
        ...
        <header className="mb-8 md:mb-12">
          <h1 className="text-3xl sm:text-5xl font-extrabold text-[#1E2A5E] mb-3 tracking-tight">Merge PDF</h1>
          <p className="text-slate-500 text-base md:text-lg font-medium opacity-80">Combine multiple PDF files into one in seconds</p>
        </header>
        ...
        <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] p-5 sm:p-14 border border-slate-100 transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(79,110,247,0.12)]">
          ...
          >
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl merge-badge-gradient flex items-center justify-center text-white shadow-xl group-hover:scale-105 transition-transform duration-300">
              <svg className="w-10 h-10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
              </svg>
            </div>
            <div className="text-center">
              <p className="text-[#1E2A5E] font-bold text-xl mb-2">Drag & drop PDF files here</p>
          ...
          {selectedFiles.length > 0 && (
            <div className="mt-10 space-y-3 max-h-64 overflow-y-auto no-scrollbar pr-2">
              {selectedFiles.map((file, index) => (
                <div key={`${file.name}-${index}`} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between transition-all hover:bg-slate-100/80">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl pdf-badge-gradient flex items-center justify-center text-white text-[8px] font-bold shadow-sm">PDF</div>
                    <div className="text-left">
                      <p className="text-[#1E2A5E] font-bold text-sm truncate max-w-[150px] sm:max-w-md">{file.name}</p>
                      <p className="text-slate-400 text-[10px] font-bold uppercase tracking-tight">{formatBytes(file.size)}</p>
                    </div>
                  </div>
                  <button 
          ...
          <div className="mt-10">
            <button 
              disabled={selectedFiles.length < 2 || status === 'processing'} 
              onClick={handleMerge}
            ...
            {selectedFiles.length === 1 && (
              <p className="text-amber-500 text-xs font-bold mt-3 animate-pulse">Select at least one more file to merge</p>
            )}
          </div>
          ...
          {status === 'processing' && (
            <div ref={progressRef} className="mt-10 p-8 bg-slate-50 border border-slate-100 rounded-[2rem] text-center transition-all duration-500">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <p className="text-[#1E2A5E] font-extrabold text-sm tracking-widest uppercase">MERGING FILES...</p>
              </div>
            ...
            <div ref={resultRef} className="mt-10 bg-[#f0fdf4] border border-[#dcfce7] rounded-[2rem] p-10 space-y-8 overflow-hidden transition-all duration-500">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div className="text-left">
                    <p className="text-[#14532d] font-extrabold text-2xl">Merged!</p>
              ...
              <div className="grid grid-cols-2 gap-px bg-[#bbf7d0] overflow-hidden rounded-2xl outline outline-1 outline-[#bbf7d0]">
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                  <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">Total Input Size</p>
                  <p className="text-[#14532d] font-extrabold text-lg leading-none">{formatBytes(result.originalTotalSize)}</p>
                </div>
                <div className="bg-white/40 p-5 backdrop-blur-sm">
                   <p className="text-[10px] text-[#15803d] font-extrabold uppercase tracking-widest opacity-60 mb-1 leading-none">Result Size</p>
          ...
          {status === 'error' && (
            <div className="mt-10 bg-[#fef2f2] border border-[#fee2e2] rounded-3xl p-8 flex items-start gap-3 transition-all duration-500">
              ...
              <div className="text-left">
                <p className="text-[#7f1d1d] font-extrabold text-lg leading-none mb-1">Merge Error</p>
